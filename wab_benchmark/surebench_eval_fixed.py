...
# -*- coding: utf-8 -*-
"""
SUREBench: Seen–Unseen Evaluation Benchmark (clean, fixed)
---------------------------------------------------------
- Naive / Rule / LM planners
- Seen-only few-shot prompting for LM (so Unseen gap emerges)
- LIBERO-style instruction CSV with optional gold actions
- Metrics: Success, Action F1, Avg Length Error, Generalization Gap

Usage examples:
  python surebench_eval_fixed.py --csv libero_instructions_patched.csv --agent rule --lang en
  python surebench_eval_fixed.py --csv libero_instructions_patched.csv --agent naive --lang en
  python surebench_eval_fixed.py --csv libero_instructions_patched.csv --agent lm --lang en --fewshot_k 8 --unseen_ratio 0.5 --lm_name google/flan-t5-small
"""
import argparse
import json
import random
...
# ---------------------------
# Parse object/target from instruction (simple heuristics)
# ---------------------------
OBJ_LEX = [
...
class BasePlanner:
    name = "base"
    def predict(self, instruction: str, **kw) -> List[str]:
...
class LMPlanner(BasePlanner):
    ...
    def predict(self, instruction: str, fewshot: str = "") -> List[str]:
        preamble = (
            "Translate the instruction into an action plan using ONLY these verbs:\n"
            "turn_on(x); open(x); close(x); pick(x); place_on(x,y); put_in(x,y).\n"
            "Rules:\n"
            "- Use lowercase.\n- Each step must be exactly one verb call like verb(arg) or verb(arg1,arg2).\n"
            "- Separate steps with semicolons only. No extra text.\n"
        )
        prompt = preamble
        if fewshot:
            prompt += "\n=== Examples (seen only) ===\n" + fewshot + "\n=== End Examples ===\n"
...
# ---------------------------
# Few-shot builder (from SEEN only)
# ---------------------------
...
# Evaluation loop
# ---------------------------
def evaluate(rows: List[Dict], idxs: List[int], planner: BasePlanner, fewshot: str = "") -> Dict:
    succ, f1s, lens = [], [], []
    for i in idxs:
...
def main():
    ...
    ap.add_argument("--csv", required=True, help="Path to LIBERO-style CSV")
    ...
    ap.add_argument("--fewshot_k", type=int, default=6, help="Few-shot examples from SEEN only for LM prompt")
    ...
    # Few-shot (Seen only) for LM
    fewshot = ""
    if isinstance(planner, LMPlanner) and seen_idxs:
    ...
    out = {
        ...
        "meta": {
            ...
            "fewshot_k": args.fewshot_k if isinstance(planner, LMPlanner) else 0
        }
    }
