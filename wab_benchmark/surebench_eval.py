...
class LMPlanner(BasePlanner):
    ...
    def predict(self, instruction: str, fewshot: str = "") -> List[str]:
        preamble = (
            "Translate the instruction into an action plan using verbs:\n"
            "turn_on(x); open(x); close(x); pick(x); place_on(x,y); put_in(x,y).\n"
            "Use lowercase, return actions separated by semicolons.\n"
        )
        prompt = preamble
        if fewshot:
            prompt += "\n=== Examples (seen only) ===\n" + fewshot + "\n=== End Examples ===\n"
...
def evaluate_with_optional_fewshot(rows, idxs, planner):
    succ, f1s, lens = [], [], []
    for i in idxs:
...
# 기존 evaluate 호출을 아래로 교체
seen_stats   = evaluate_with_optional_fewshot(rows, seen_idxs, planner)
unseen_stats = evaluate_with_optional_fewshot(rows, unseen_idxs, planner)
overall_stats= evaluate_with_optional_fewshot(rows, list(range(len(rows))), planner)
